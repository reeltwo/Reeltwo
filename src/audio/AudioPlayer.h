...
public:
    ...
    size_t   init(size_t bufSize = 0);  // set default values
...
typedef void (*AudioSampleFilter)(unsigned numBits, unsigned numChannels, const int16_t* samples, unsigned sampleCount);

class AudioPlayer : private AudioBuffer{
...
public:
    ...
    inline void setSampleFilter(AudioSampleFilter filter)
    {
        sampleFilter = filter;
    }
...
private:
    ...
    virtual void audio_info(const char *info) {}
    virtual void audio_id3data(const char *info) {}
    virtual void audio_id3image(File& file, const size_t pos, const size_t size) {}
    virtual void audio_eof_mp3(const char *info) {}
    virtual void audio_showstation(const char *info) {}
    virtual void audio_showstreamtitle(const char *info) {}
    virtual void audio_bitrate(const char *info) {}
    virtual void audio_commercial(const char *info) {}
    virtual void audio_icyurl(const char *info) {}
    virtual void audio_lasthost(const char *info) {}
    virtual void audio_eof_speech(const char *info) {}
...
private:
    ...
    bool            m_f_nullstream = false ;        // Play from null
    bool            m_f_ssl = false;
