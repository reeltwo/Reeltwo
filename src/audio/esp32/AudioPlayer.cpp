...
size_t AudioBuffer::init(size_t bufSize) {
    if(psramInit()) {
        // PSRAM found, AudioBuffer will be allocated in PSRAM
        m_buffSize = (bufSize != 0) ? bufSize : m_buffSizePSRAM;
        if(m_buffer == NULL) {
        ...
        m_buffSize = (bufSize != 0) ? bufSize : m_buffSizeRAM;
        m_buffer = (uint8_t*) calloc(m_buffSize, sizeof(uint8_t));
        m_buffSize = m_buffSizeRAM - m_resBuffSize;
    }
    if(!m_buffer)
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::connecttohost(const char* host, const char* user, const char* pwd) {
    ...
    if(m_f_ssl == false) {
        if(client.connect(hostwoext.c_str(), port)) {
            audio_info("Connected to server");
            client.print(resp);
    ...
    if(m_f_ssl == true) {
        if(clientsecure.connect(hostwoext.c_str(), port)) {
            // audio_info("SSL/TLS Connected to server");
...
//TEST loop
bool AudioPlayer::setFileLoop(bool input){
    m_f_loop = input;
    return input;
}
//-----------------------------------------------------------------------------------------------------------------------------------

bool AudioPlayer::connecttoSD(const char* sdfile) {
    return connecttoFS(SD, sdfile);
}
//-----------------------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::connecttoFS(fs::FS &fs, const char* file) {
    ...
    audio_info("Neither wave nor mp3 format found");
    return false;
}
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::connecttospeech(const char* speech, const char* lang){
...
bool AudioPlayer::connecttonull() {
    ...
    m_bitRate = 64000;//getChannels() * getSampleRate() * getBitsPerSample();
...
//---------------------------------------------------------------------------------------------------------------------
String AudioPlayer::urlencode(String str) {
...
//---------------------------------------------------------------------------------------------------------------------
int AudioPlayer::readWaveHeader(uint8_t* data, size_t len) {
...
//---------------------------------------------------------------------------------------------------------------------
int AudioPlayer::readID3Metadata(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == 6){ // Read the value
        ...
        if(chbuf[0] != 0) audio_id3data(chbuf);
...
//---------------------------------------------------------------------------------------------------------------------
int AudioPlayer::readM4AContainer(uint8_t* data, size_t len){ // todo read header
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::playChunk() {
    if (sampleFilter != NULL)
    {
        sampleFilter(getBitsPerSample(), m_channels, &m_outBuff[m_curSample], m_validSamples);
    }
...
void AudioPlayer::loop() {
    ...
    // - webstream - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    else if(m_f_webstream) {                                      // Playing file from URL?
        processWebStream();
    }
    // - webstream - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    else if(m_f_nullstream) {                                     // Playing file from audio filter?
        processNullStream();
    }
}
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::processLocalFile() {
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::processWebStream() {
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::processNullStream() {
    if (m_f_running && m_f_nullstream) {
        if(getBitsPerSample() > 8) memset(m_outBuff,   0, sizeof(m_outBuff));     //Clear OutputBuffer (signed)
        else                       memset(m_outBuff, 128, sizeof(m_outBuff));     //Clear OutputBuffer (unsigned, PCM 8u)

        m_validSamples = 256;
        m_curSample = 0;
        compute_audioCurrentTime(m_validSamples * 2);
        while (m_validSamples)
            playChunk();
    }
}
//----------------------------------------------------------------------------------------------------------------------
void AudioPlayer::processControlData(uint8_t b) {
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::parsePlaylistData(const char* pd){
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::parseAudioHeader(const char* ah) {
    char ah_buff[512];
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::parseContentType(const char* ct) {
    bool ct_seen = false;
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::showstreamtitle(const char* ml) {
    // example for ml:
...
//---------------------------------------------------------------------------------------------------------------------
int AudioPlayer::sendBytes(uint8_t* data, size_t len) {
    ...
    if(!m_f_playing) {
        ...
        if(nextSync == -1) {
            if(swnf<1) audio_info("syncword not found");
            ...
            audio_info("decoder syncword not found, unknown reason");
            return -1;
        }
    }
    ...
    if(ret) { // Error, skip the frame...
        ...
        if(m_codec == CODEC_MP3){
            ...
            if((int) MP3GetSampRate() ==0){ // error can't be
                audio_info("SampleRate=0, try new frame");
                m_f_playing = false;
                return 1;
            }
        ...
        if(m_codec == CODEC_AAC || m_codec == CODEC_M4A){
            ...
            if(AACGetSampRate() == 0){ // error can't be
                audio_info("AAC SampleRate=0, try new frame");
                m_f_playing = false;
                return 1;
            }
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::compute_audioCurrentTime(int bd) {
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::printDecodeError(int r) {
    String e = "";
    if(m_codec == CODEC_MP3){
        switch(r){
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t AudioPlayer::getAudioCurrentTime() {  // return current time in seconds
    return (uint32_t) m_audioCurrentTime;
}
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::setFilePos(uint32_t pos) {
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::audioFileSeek(const int8_t speed) {
    bool retVal = false;
    if(audiofile && speed) {
        retVal = true; //    
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::setBitsPerSample(int bits) {
...
//---------------------------------------------------------------------------------------------------------------------
bool AudioPlayer::setChannels(int ch) {
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::setTone(uint8_t l_type, uint16_t l_freq, uint8_t r_type, uint16_t r_freq){
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::IIR_calculateCoefficients(){  // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* AudioPlayer::IIR_filterChain(int16_t iir_in[2], bool clear){  // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::forceMono(bool m) { // #100 mono option
    m_f_forceMono = m; // false stereo, true mono
}
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::setBalance(int8_t bal){ // bal -16...16
    if(bal < -16) bal = -16;
    if(bal >  16) bal =  16;
    m_balance = bal;
}
//---------------------------------------------------------------------------------------------------------------------
void AudioPlayer::setVolume(uint8_t vol) { // vol 22 steps, 0...21
    if(vol > 21) vol = 21;
...
//---------------------------------------------------------------------------------------------------------------------
int32_t AudioPlayer::Gain(int16_t s[2]) {
    int32_t v[2];
